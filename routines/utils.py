...
- print_error(message, level="Warning") -> None
- print_list(lista) -> None
- requires_jquery_ui(request) -> Boolean
- month_name(month) -> String(3)
- unaccent(name) -> String
- clean_name(name, to_lower=True) -> String
- move_uploaded_file(file, upload_to) -> String
- is_mobile(request) -> Boolean
- get_setting_fn(sectionvalue, Setting)
- as_paragraph_fn(text)
- hipernormalize(text=None)
- truncate(f, n=0)
- send_mail(
        asunto, texto_plano, email_from, email_to, texto_html, imagenes=())
- inter_periods_days(p1_inicio, p1_fin, p2_inicio, p2_fin)
- free_days(p1_inicio, p1_fin, p2_inicio, p2_fin)
- create_date_range(inicio, fin)
- BootstrapColors
"""
...
def unaccent(name):
    """
    Limpia name de acentos, dieresis y ñ

    (string) name -> cadena a limpiar
...
def clean_name(name, to_lower=True):
    ...
    return string
    """
    name = unaccent(name.replace(" ", "_"))
    return name.lower() if to_lower is True else name
